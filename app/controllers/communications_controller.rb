    ...
    if params["new_people"].present?
      params["new_people"].each do |index_number,information|
        if (information["medium"].present? and information["contents"].present?)
          ...
          #get rid of autogenerated reminders if communication takes place
          reminder_event = Event.where(event_type: "auto_generated_communication_reminder", person_id: id, dismissed: false)
          if (information["medium"] != "email" and reminder_event.present?)
            re = reminder_event[0]
            re.dismissed = true
            re.dismissed_reason = "communication_happened"
            re.save
          end
